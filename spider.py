...
## create data table
names = ['title', 'event/rate', 'wangpiao', 'hapiao', 'wanda', 'jinyi', 'realtime', 'forecast', 'total']
data = [] #final json list
for item in items:
    #for each item, split by td
    #follow as title(电影名称), 总场次/占比, 网票票房, 哈票, 万达, 金逸, 实时, 预计, 累积
    #mark-- as 0--------------, 1----------, 2-------, 3---, 4---, 5---, 6---, 7---, 8---
    ...
    idx = 2 #[2, ..., 8]
    while idx <= 8:
        answer[names[idx]] = str2db(tds[idx][4 : (len(tds[idx]) - 5)])
        idx = idx + 1
    data.append(answer)
